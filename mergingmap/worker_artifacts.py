...
MergingMap 测试工作器的输出记录与动态区域辅助函数。
"""
...
class WorkerArtifactMixin:
    ...
    def _save_initial_direction_record(self) -> str:
        output_root = Path(self.output_root)
    ...
    def _region_event_path(self) -> str:
        output_root = Path(self.output_root)
    ...
    # 中文目的：将区域分配器产生的事件追加到 JSONL 日志。
    # English purpose: Append coordinator events to the JSONL log.
    def _write_region_events(self) -> None:
