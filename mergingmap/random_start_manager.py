"""Deterministic random-start generation for reproducible matched experiments."""
...
def extract_ground_truth_grid(env):
...
def _world_to_cell(env, position):
...
def validate_random_starts(env, n_agents, config):
...
def derived_attempt_seed(base_seed, attempt_index):
...
def create_environment_with_valid_random_starts(env_factory, base_seed, n_agents, config):
