...
# Purpose: convert map grid coordinates to environment world coordinates.
# English purpose: Convert a map cell into an environment world coordinate.
def _cell_to_world(env, cell):
...
# Purpose: identify free cells adjacent to unknown area.
# English purpose: Identify free cells adjacent to unknown space.
def _frontier_mask(occupancy, free_value, unknown_value):
...
# Purpose: extract deterministic eight-connected components from the frontier mask.
...
# Purpose: choose spatially separated deterministic partition seeds.
# English purpose: Choose deterministic spatially separated partition seeds.
def _farthest_seeds(cells, count):
...
# Purpose: split oversized frontier components into smaller regions.
# English purpose: Split an oversized frontier component into smaller regions.
def _partition_component(cells, max_cells):
...
# Purpose: derive a stable region id from quantised position and bounding box.
# English purpose: Create a stable region ID from quantised location and bounds.
def _make_region_id(cells, quantization_cells):
...
# Purpose: extract dynamic frontier tasks from the robot's current fused occupancy map.
# English purpose: Extract dynamic frontier tasks from a robot merged occupancy belief.
def extract_frontier_regions(occupancy, env, config):
...
# Purpose: keep stable identity of dynamic frontier regions across consecutive planning steps.
# English purpose: Preserve dynamic frontier identity across consecutive planning steps.
def track_frontier_regions(previous_regions, current_regions, config):
    """Greedily match new frontiers to old identities by IoU and centroid distance.

    Scores every admissible old/new pair with
    ``w_I * IoU - w_mu * normalised_distance``, performs a deterministic
    one-to-one greedy match, and copies the old identifier (``region_id``) onto
    the matched new geometry so ages, leases, and progress records survive small
    changes.
    """
...
# Purpose: compute the intersection-over-union of two regions' frontier cell sets.
...
# Purpose: compute the grid distance between two region centroids.
# English purpose: Compute cell-space distance between region centroids.
def _centroid_cell_distance(first, second):
    return float(
        np.linalg.norm(
...
# Purpose: compute the Euclidean distance between two world coordinates.
# English purpose: Compute Euclidean distance between world coordinates.
def _world_distance(first, second):
    return float(
        np.linalg.norm(
...
# Purpose: decide whether two region observations represent the same evolving task.
# English purpose: Determine whether two regions represent the same evolving task.
def _regions_equivalent(first, second, config):
...
# Purpose: decide whether a local region conflicts with a known peer claim.
# English purpose: Determine whether a local region matches a known peer claim.
def _region_matches_claim(region, claim, config):
...
# Purpose: convert contact edges into robot connected components.
# English purpose: Convert contact edges into robot connected components.
def _contact_components(n_robots, contact_pairs):
