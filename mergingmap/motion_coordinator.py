...
MergingMap 面向短时运动交换、碰撞消解与死锁恢复的统一接口。
"""
...
class MotionDecision:
    ...
    中文目的：把首选动作、最终动作、阻塞机器人、租约释放请求与诊断信息作为
    整体返回测试循环。English implementation: packages executable positions and
...
class MergingMapMotionCoordinator:
    ...
    中文目的：在不修改冻结 DARE 的前提下，依次执行一到两步运动意图交换、局部
    预留过滤、同步冲突消解、动态优先级、振荡软惩罚与分阶段恢复。
    中文实现：所有硬约束始终由 collision 模块检查；deadlock 模块仅改变候选顺序、
    请求释放租约或选择图上回退第一步，绝不生成图外动作或放宽碰撞约束。
    ...
    def __init__(
        ...
        *,
        safe_distance: float,
        mode: CoordinationMode = "collision_deadlock",
        deadlock_wait_threshold: int = 3,
        deadlock_stall_threshold: int | None = None,
        deadlock_soft_relax_threshold: int | None = None,
        deadlock_lease_release_threshold: int | None = None,
        deadlock_backtrack_threshold: int | None = None,
        deadlock_wait_weight: float = 1.0,
        deadlock_stall_weight: float = 1.0,
        max_backtracking_nodes: int = 20000,
        node_resolution: float = 1.0,
        motion_reservation_horizon: int = 2,
        motion_cache_ttl_steps: int = 2,
        motion_packet_header_bytes: int = 40,
        oscillation_base_penalty: float = 2.0,
        oscillation_repeat_penalty: float = 1.0,
    ) -> None:
    ...
    @property
    def serial_fallback_events(self) -> int:
    ...
    @staticmethod
    def _copy_candidate_lists(
        values: Sequence[Iterable[Sequence[float]]],
    ...
    def _apply_recovery_candidates(
        self,
        *,
        current_positions: Sequence[Sequence[float]],
        candidates: list[list[np.ndarray]],
        recovery_candidate_lists: Sequence[Iterable[Sequence[float]]] | None,
        stages: Sequence[int],
        robots: Sequence[object] | None,
    ) -> tuple[list[list[np.ndarray]], tuple[int, ...], tuple[int, ...], tuple[int, ...]]:
    ...
    def resolve_step(
        ...
        *,
        ...
        short_horizon_plans: Sequence[Sequence[Sequence[float]]] | None = None,
        recovery_candidate_lists: Sequence[Iterable[Sequence[float]]] | None = None,
        stall_steps: Sequence[int] | None = None,
        robots: Sequence[object] | None = None,
    ) -> MotionDecision:
        """Resolve one graph-legal multi-robot motion step.

        中文目的：为测试循环提供唯一动作协调入口，并按论文顺序执行短意图交换、
        硬安全过滤、动态优先级与分阶段恢复。English implementation: keeps ghost
