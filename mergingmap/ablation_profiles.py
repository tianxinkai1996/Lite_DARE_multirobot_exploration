...
class MergingMapAblationProfile:
    ...
    English implementation: stores immutable layer
    ...
    def with_extra_supervisors(self, enabled):
...
# reintroduced. Legacy command-line names are kept only as configuration aliases.
...
def method_display_name(model_name, method_key):
...
def resolve_ablation_profile(value, include_extra_supervisors=False):
