...
class MergedBeliefManager(MergedBeliefDiagnosticsMixin):
    ...
    def __init__(self, local_maps, config):
        if not local_maps:
    ...
    def sent_mask_memory_bytes(self):
    ...
    def _mark_new_information(self, robot_id, flat_indices, source_peer=None):
    ...
    def _resolve_conflicts(self, existing, incoming):
    ...
    def sync_local_map(self, robot_id, local_map):
    ...
    def _record_encoded_packet(self, packet):
        self.packets_sent += 1
    ...
    def apply_packet(self, receiver_id, packet):
    ...
    def _deliver(self, receiver_id, packet):
    ...
    def exchange_pair(self, robot_i, robot_j, step, mode="delta"):
    ...
    def merged_map(self, robot_id, copy_map=True):
        robot_id = self._validate_robot_id(robot_id)
        return self._maps[robot_id].copy() if copy_map else self._maps[robot_id]

    def make_map_info(self, robot_id, template_map_info):
