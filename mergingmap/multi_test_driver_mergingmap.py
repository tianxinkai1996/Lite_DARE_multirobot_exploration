...
def load_frozen_policy(device: torch.device, checkpoint_path: str | Path | None = None):
    import dill
    import hydra
...
def parse_map_selection(value: str, map_count: int | None) -> list[int]:
    """Parse ``all``, comma-separated indices, and inclusive ranges.

    中文目的：允许命令行直接选择单张地图或 ``3-8`` 这样的连续地图范围。
...
def run_seed(
    ...
    *,
    ...
    中文目的：第4章只设置一个基础 random seed；不同地图和重复 trial 使用
    可复现的确定性偏移，避免重复运行得到完全相同的随机场景。
