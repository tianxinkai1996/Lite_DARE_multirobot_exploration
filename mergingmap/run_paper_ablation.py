...
运行受控的 MergingMap、碰撞与死锁论文消融。原始 DARE 机器人基线由
``run_original_dare_reference.py`` 使用同一场景清单单独执行。
"""
...
def _write_csv(path: Path, rows: Sequence[Mapping]) -> None:
    ...
    中文目的：持续保存实验与场景清单，即使中途停止也保留已完成结果。
...
def _parse_positive_ints(value: str, *, option: str) -> tuple[int, ...]:
    ...
    中文目的：统一解析机器人数量等命令行列表，并拒绝零、负数和重复值。
...
def scenario_id(
    ...
    中文目的：使原 DARE、MM、碰撞和死锁结果可按完全相同场景配对。
    English implementation: encodes map, repeat, team size, deterministic seed,
    and map-communication treatment in one stable human-readable identifier.
    """
...
def _normalise_start_positions(value: object) -> str:
    """Canonicalise JSON start positions for cross-method integrity checks.

    中文目的：检测同一场景是否因配置差异产生了不同起点。
...
def build_parser() -> argparse.ArgumentParser:
    ...
    中文目的：集中暴露方法、地图、重复次数、机器人数量和输出路径。
...
def _episode_process_init(
    checkpoint_path: str,
    methods_root: str,
    include_extra_supervisors: bool,
    save_visualisation: bool,
) -> None:
    """Initialise one reusable policy inside a spawned episode process.

    中文目的：每个并行进程只加载一次冻结策略，避免每个 episode 重复加载
    checkpoint；使用 ``spawn`` 避免 fork 已初始化 CUDA 的父进程。
    English: one policy/CUDA context is retained per spawned process and reused
    for every episode assigned to that process.
    """
...
def _run_parallel_episode(row: Mapping[str, int | str]) -> dict[str, object]:
    """Execute one independent multi-robot episode in a child process.

    中文目的：子进程只负责 episode 计算和该 episode 的独立 artifact；聚合 CSV、
    场景一致性检查和失败日志仍由父进程单独写入，避免并发写文件冲突。
    English: workers compute episodes and unique per-episode artifacts only; the
    parent process remains the sole writer of aggregate tables and manifests.
    """
...
def _policy_metadata(checkpoint_path: Path, run_root: Path) -> dict[str, object]:
...
def _planned_rows(
    *,
    ...
    maps: Sequence[int],
    default_trials: int,
    trial_overrides: Mapping[int, int],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    中文目的：一次加载冻结策略，依次运行三种模块组合并验证相同场景起点一致。
    ...
    def handle_success(row: Mapping[str, int | str], metrics: Mapping[str, object]) -> None:
        nonlocal completed
        _consume_success(
            row=row,
            metrics=metrics,
    ...
    def handle_failure(row: Mapping[str, int | str], exc: BaseException) -> None:
        failure = {
            **dict(row),
