...
MergingMap 使用的冻结 DARE 图构建与推理辅助函数。
"""
...
class WorkerDareRuntimeMixin:
    ...
    def _base_dare_candidates(
        ...
        中文目的：为恢复阶段保留一个不含初始方向、区域与振荡软代价的候选列表。
    ...
    def _apply_candidate_supervisors(
        self,
        robot_id: int,
        candidates: List[np.ndarray],
        *,
        ...
        中文目的：把任务层监督器与纯 DARE 排序解耦，使死锁恢复能够暂时移除所有
        软代价但仍只使用原始图邻居。English implementation: applies supervisors
    ...
    def _short_horizon_plan(
        ...
        中文目的：按方法章节只交换短时意图，而非完整 DARE 轨迹；第一步为当前
        首选合法候选，第二步从第一步图节点的邻居中按下一预测方向选择。
