...
class MergedBeliefDiagnosticsMixin:
    ...
    def known_cell_count(self, robot_id: int) -> int:
    ...
    def pairwise_agreement(self, robot_i: int, robot_j: int) -> float:
    ...
    def min_pairwise_agreement(self) -> float:
        values = [
            self.pairwise_agreement(i, j)
            for i in range(self.n_robots)
            for j in range(i + 1, self.n_robots)
