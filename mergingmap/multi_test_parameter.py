...
# Reliability experiments default to synchronous loss-free operation; additional
# paper experiments may override this via environment variables.
...
# Collision/deadlock ablations and experiment-metric recording.
# GHOST_MODE=True: no coordination.
# GHOST_MODE=False and ENABLE_COLLISION_DEADLOCK_AVOIDANCE=False:
# collision-only: resolve current-step collisions only.
# Both flags enable full collision + persistent deadlock handling.
# Both flags enable collision resolution plus persistent deadlock handling.
