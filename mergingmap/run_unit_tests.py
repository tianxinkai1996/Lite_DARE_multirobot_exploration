...
# 中文目的：执行命令并把合并输出保存到指定文件。
# English purpose: Execute a command and persist its combined output.
def run_and_record(command: list[str], output_path: Path) -> int:
...
# 中文目的：运行 MergingMap 测试、模块语法检查并写出汇总。
...
def main() -> int:
    ...
    # 编译 MergingMap 以及拆分后的 collision/deadlock 模块。
    syntax_paths = [
