...
class MultiRobotTestWorker:
    ...
    def __init__(
        ...
        *,
        ...
        start_sample: int = 0,
        visual_output_root: str | None = None,
        test_profile: str | DareTestProfile = "coordinated",
    ...
    def _communication_cumulative(self) -> dict[str, int | float]:
        """Return monotonic communication counters in the shared schema.

        中文目的：记录原 DARE 消息的负载、编码、接收和进程内传递耗时。
    ...
    def _predicted_world_plan(self, robot_id: int, action_pred: np.ndarray) -> List[np.ndarray]:
    ...
    def _ordered_dare_candidates(self, robot_id: int, action_pred: np.ndarray) -> List[np.ndarray]:
    ...
    def _exchange_messages(
        ...
        *,
        ...
        dare_plans: Sequence[Sequence[np.ndarray]],
    ...
    def _update_next_observations(self) -> None:
        for robot_id in range(self.n_agents):
            self._update_robot_graph(robot_id)
    ...
    def run_episode(self) -> Dict[str, float | int | str | bool]:
        ...
        中文目的：在不改变 DARE 策略输入和动作的前提下记录所有可行的计算、
        通信、内存、能耗、冲突与探索指标。
