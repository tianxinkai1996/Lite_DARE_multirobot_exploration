...
def _fake_env(grid: np.ndarray, cell_size: float = 1.0) -> MultiRobotEnv:
...
def test_clear_centre_is_accepted() -> None:
    grid = np.full((7, 7), FREE, dtype=np.int32)
    env = _fake_env(grid)
    assert env.is_start_clear_world((3.0, 3.0), 0.6)


def test_wall_intersection_is_rejected() -> None:
    grid = np.full((7, 7), FREE, dtype=np.int32)
    grid[3, 4] = OCCUPIED
    env = _fake_env(grid)
...
def test_map_boundary_is_rejected() -> None:
    grid = np.full((7, 7), FREE, dtype=np.int32)
    env = _fake_env(grid)
    assert not env.is_start_clear_world((0.0, 0.0), 0.6)


def test_occupied_centre_is_rejected() -> None:
    grid = np.full((7, 7), FREE, dtype=np.int32)
    grid[3, 3] = OCCUPIED
    env = _fake_env(grid)
