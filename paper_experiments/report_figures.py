...
def _model_parameter_count(part, model_name):
...
def _save_figure(fig, out, stem, preview_png=True):
...
def _metric_numeric(series, metric):
...
def _bootstrap_macro_ci(groups, samples, seed):
...
def _summary_overall_and_team(data, treatment_column, treatments, metrics, samples, seed):
    rows: list[dict] = []
...
def _pivot_summary(long_summary, group_columns):
...
def _paired_effects_overall_and_team(data, treatment_column, reference, candidates, metrics, samples, seed):
...
def _quantile_table(data, group_columns, metrics):
    work = _add_derived_metrics(data)
    rows: list[dict] = []
...
def _scaling_table(summary_long, treatment_column):
    rows: list[dict] = []
...
def _single_robot_absolute_table(single):
...
def table_4_2_selection(selection_file, out, notes):
...
def figure_4_1(single, out, notes):
...
def figure_4_2(single, out, notes, samples, seed):
...
def _multi_metric_columns(work):
    final_cov = _first(work, ["final_coverage", "team_coverage"])
...
def _summary_metric(summary, metric, target):
...
def _wide_effect_table(effects, mapping):
    rows = []
...
def _plot_treatment_scaling(ax, long_summary, metric, treatment_column, treatments, labels, ylabel, percent=False):
...
def figure_4_3(multi, out, notes, samples, seed):
...
def _communication_metric_columns(work):
    final_cov = _first(work, ["final_coverage", "team_coverage"])
...
def _communication_savings(work):
...
def _communication_common(comm, samples, seed):
...
def figure_4_5(comm, out, notes, samples, seed):
...
def figure_4_6(comm, out, notes, samples, seed):
    ...
    def _point(scope_label, team_size, mode, metric):
        part = long_summary.loc[
...
def _case_interaction_scores(work):
    work = _add_derived_metrics(work)
...
def _select_qualitative_cases(multi, comm):
    rows: list[dict] = []
...
def _pair_rows(data, pair_key_value, column, values):
...
def _explicit_image_path(row, root):
...
def _discover_image(row, root, image_index):
...
def _plot_trajectory_comparison(ax, rows, labels, title):
    plotted = False
    linestyles = ("--", "-")
...
def _show_image_pair(ax, rows, labels, root, image_index, title):
    found = []
    for label in labels:
        row = rows.get(label)
        if row is None:
...
def _plot_communication_history(ax, rows):
    plotted = False
...
def figure_4_7(multi, comm, root, out, notes):
...
def _win_tie_loss_table(data, treatment_column, reference, candidate, metrics):
    if data.empty or "_report_pair" not in data.columns:
        return pd.DataFrame()
    work = _add_derived_metrics(data)
    rows=[]
...
def _map_level_effects(data, treatment_column, reference, candidate, metrics):
...
def _difficulty_tables(data, treatment_column, metrics):
...
def _payload_correlations(data, iou_metric):
    work=_add_derived_metrics(data)
