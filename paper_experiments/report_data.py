...
def _normalise_mode(data: pd.DataFrame) -> None:
...
def read_all_results(root: Path) -> pd.DataFrame:
...
def numeric_columns(data: pd.DataFrame, columns: Iterable[str]) -> None:
    for column in columns:
        if column in data.columns:
            data[column] = pd.to_numeric(data[column], errors="coerce")


def _team_size_mask(data: pd.DataFrame) -> pd.Series:
    if "team_size" not in data.columns:
        return pd.Series(False, index=data.index)
    return pd.to_numeric(data["team_size"], errors="coerce").isin(PRIMARY_TEAM_SIZES)


def primary_single(data: pd.DataFrame) -> pd.DataFrame:
...
def primary_multi(data: pd.DataFrame) -> pd.DataFrame:
    mask = (
...
def primary_communication(data: pd.DataFrame) -> pd.DataFrame:
    mask = (
...
def partition_reporting_scope(data: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def comparison_pair_key(data: pd.DataFrame) -> pd.Series:
...
def _normalise_start_positions(series: pd.Series) -> pd.Series:
    return (
        series.fillna("")
        .astype(str)
...
def complete_multi_pairs(data: pd.DataFrame) -> pd.DataFrame:
...
def complete_communication_pairs(data: pd.DataFrame) -> pd.DataFrame:
    return complete_treatment_pairs(
        data,
...
def coverage_report(data: pd.DataFrame) -> dict:
