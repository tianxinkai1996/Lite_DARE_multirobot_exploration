...
class EpisodeMetricsRecorder:
    ...
    def __init__(
        ...
        *,
        ...
        initial_known_free_cells: int,
        node_resolution: float,
        safe_distance: float,
        deadlock_wait_threshold: int = 3,
        coverage_thresholds: Sequence[float] = (0.90, 0.95, 0.99),
    ) -> None:
        if team_size <= 0:
    ...
    @staticmethod
    def known_free_cell_count(env, free_value: int) -> int:
    ...
    def map_quality_metrics(
        ...
        *,
        ...
        中文目的：量化 MergingMap 是否提高已知区域、自由空间和障碍物地图质量。
        中文实现：对团队地图采用保守融合（任一障碍报告优先，其次自由，否则未知），
        并分别计算 known accuracy、precision、recall、F1 与 IoU。

        English implementation: forms a conservative team union, evaluates free
        and occupied classes against ground truth, and averages the same metrics
        across individual robot beliefs.
        """
        ...
        def evaluate(prediction: np.ndarray) -> dict[str, float]:
    ...
    def map_structure_metrics(
        ...
        *,
        ...
        中文目的：为 Figure 4-8 提供与算法结果无关的地图结构特征，避免使用
        碰撞结果反向定义“困难场景”。中文实现：统计障碍占比、自由栅格的四邻域
        度数，并把度数不超过 2 的自由栅格作为狭窄/走廊代理量。

        English implementation: derives obstacle ratio and a topology-only narrow
        free-cell proxy from four-neighbour connectivity. Difficulty tiers are assigned
        later across the dataset, so no outcome metric leaks into the stratification.
        """
    ...
    def reset_wall_clock(self) -> None:
        ...
        中文目的：排除环境初始化和一次性 FLOPs profiling，使回合总耗时可公平比较。
        English implementation: restarts only the wall timer while preserving all
        already configured metric definitions and initial exploration state.
        """

        self.started_at = time.perf_counter()

    def _node_key(self, position: Sequence[float]) -> tuple[int, int]:
    ...
    def _register_visit(self, robot_id: int, position: Sequence[float]) -> tuple[bool, bool]:
    ...
    def _trajectory_row(
        ...
        *,
        ...
        static_blocked: bool,
        consecutive_wait_steps: int,
        deadlock_flag: bool,
        recovered_flag: bool,
        revisit_flag: bool,
        cross_robot_overlap_flag: bool,
    ) -> dict:
    ...
    def _append_event(self, *, step: int, event: Mapping) -> None:
    ...
    def record_step(
        ...
        *,
        step: int,
        preferred_positions: Sequence[Sequence[float]],
        proposed_positions: Sequence[Sequence[float]],
        actual_positions: Sequence[Sequence[float]],
        coverage: float,
        known_free_cells: int,
        dynamic_blocked_robot_ids: Iterable[int] = (),
        static_blocked_robot_ids: Iterable[int] = (),
        contact_pairs: Sequence[tuple[int, int]] = (),
        map_packets_cumulative: int = 0,
        map_bytes_cumulative: int = 0,
        communication_cumulative: Mapping[str, int | float] | None = None,
        resolver_info: object | None = None,
        step_runtime_ms: float = 0.0,
        extra_step_metrics: Mapping[str, object] | None = None,
    ) -> None:
        """Record one executed synchronous team step.

        中文目的：统一保存探索、通信、模块耗时和资源占用的逐步数据。
        English implementation: combines core exploration fields with optional
        monotonic communication counters and detailed runtime/resource samples.
        """
    ...
    def finalise(self, *, success: bool, extra_metrics: Mapping | None = None) -> dict:
