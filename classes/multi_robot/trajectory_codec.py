"""Compact binary codecs for short-horizon communication.

Encodes/decodes the short future plan, recent trail and sparse coverage
tiles exchanged between robots as small RLE-compressed byte blobs, plus the
packet-level budget trimming used to respect a fixed message size limit.
"""
...
def encode_path_rle(points, node_resolution):
...
def decode_path_rle(payload, node_resolution):
...
def encode_tiles_delta_rle(tiles):
...
def decode_tiles_delta_rle(payload):
...
def _trim_to_budget(trail, plan, coverage_tiles, goal_tile, node_resolution, budget):
...
def make_compressed_packet(sender_id, step, current_position, trail, plan, node_resolution, trail_steps, plan_steps, packet_budget_bytes, coverage_tiles=None, goal_tile=None):
...
def make_raw_packet(sender_id, step, current_position, trail, plan, trail_steps, plan_steps, coverage_tiles=None, goal_tile=None):
...
def decode_packet(packet, node_resolution):
