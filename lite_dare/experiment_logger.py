"""Structured logging of LiteDARE training and evaluation runs."""
...
def safe_git_commit(project_root):
    try:
...
def collect_environment_info(project_root):
...
def find_torch_modules(workspace):
...
def parameter_statistics(module):
...
def collect_parameter_statistics(workspace):
...
def write_run_manifest(run_dir, command, environment):
    write_json(
