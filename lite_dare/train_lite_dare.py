"""Train LiteDARE variants (2/4-layer graph encoders) on the DARE maze dataset."""
...
def resolve_project_paths(args):
...
def compose_discrete_config(config_file, config_dir, overrides):
    """
...
def validate_discrete_config(cfg):
...
def apply_lite_encoder(cfg, encoder_layers, encoder_heads):
...
def apply_retraining_profile(cfg, args):
    """
...
def create_run_dir(output_root, architecture, seed):
...
def update_logging_metadata(cfg, architecture):
...
def save_architecture_metadata(cfg, run_dir, architecture, source_config_file, args):
    OmegaConf.save(
        config=cfg,
...
def print_summary(cfg, architecture, config_file, run_dir):
    print("=" * 78)
...
def construct_workspace(cfg, run_dir):
